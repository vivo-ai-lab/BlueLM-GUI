...
@app.post("/close_app")
async def close_app_endpoint(app_name: str, app_android_env: AndroidEnv):
    try:
        real_env = app_android_env.controller.env
    ...
    try:
        result = await loop.run_in_executor(
            None, partial(adb_utils.close_app,app_name,real_env)
            )
        if result:
            return{
                "status": "success",
                "message": f"App '{app_name}' closed successfully."
            }
        else:
            return{
                "status": "failure",
...
def numpy_to_base64(image_np):
    image = Image.fromarray(image_np)          # 按 RGB 处理
    buf = io.BytesIO()
...
async def get_screenshot(wait_to_stabilize: bool, app_android_env: AndroidEnv):
  ...
  screen_base64 = numpy_to_base64(state.pixels)
  # return screen_base64
  return {"pixels": screen_base64}
