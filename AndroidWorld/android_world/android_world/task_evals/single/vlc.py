...
class _VLC(task_eval.TaskEval):
  ...
  @classmethod
  def _reset_vlc_first_launch(cls, env: interface.AsyncEnv):
    """Resets VLC to a pristine first-launch state with storage permission.

    VlcApp.setup launches VLC once before any permission is granted. That
    first launch initializes the medialibrary WITHOUT storage access, which
    leaves its media-library entry points unregistered - the Video tab then
    shows "No media files found" until the user manually enables a folder in
    Media Library preferences. Granting the permission afterwards does NOT
    repair this; VLC never re-registers on its own.

    The fix: wipe VLC's app data (pm clear), grant the permission, then run
    one clean first launch via monkey (the launcher path initializes the
    medialibrary correctly; "am start -n" does not). After this, VLC's
    medialibrary is healthy and any later launch shows the scanned videos.
    """
    pkg = 'org.videolan.vlc'
    adb_utils.issue_generic_request(
        ['shell', 'pm', 'clear', pkg], env.controller
    )
    adb_utils.issue_generic_request(
        ['shell', 'pm', 'grant', pkg,
    ...
    # First launch with permission: triggers medialibrary init + storage
    # registration + a full scan.
    adb_utils.issue_generic_request(
        ['shell', 'monkey', '-p', pkg,
         '-c', 'android.intent.category.LAUNCHER', '1'],
        env.controller,
    )
    # time.sleep(5)  # Let the first-launch scan finish.
    time.sleep(3)
    # Dismiss the first-run onboarding wizard that appears after pm clear.
    # These three taps correspond to the onboarding flow on VLC 3.5.x.
    for x, y in [(645, 2232), (957, 2231), (540, 1303)]:
      adb_utils.issue_generic_request(['shell', 'input', 'tap', str(x), str(y)], env.controller)
      time.sleep(2)
    time.sleep(3) 
    adb_utils.issue_generic_request(
        ['shell', 'am', 'force-stop', pkg], env.controller
    )
...
class VlcCreatePlaylist(_VLC):
  ...
  def initialize_task(self, env: interface.AsyncEnv):
    ...
    # Files were just generated. Reset VLC to a pristine first-launch state
    # (granting storage permission this time) so its medialibrary scans and
    # indexes them on its very first proper launch - otherwise the Video tab
    # shows "No media files found" until a folder is manually enabled.
    self._reset_vlc_first_launch(env)
