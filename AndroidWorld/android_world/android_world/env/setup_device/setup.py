...
def setup_app(app: Type[apps.AppSetup], env: interface.AsyncEnv) -> None:
  """Sets up a single app, retrying up to 3 times on failure."""
  for attempt in range(3):
    try:
      logging.info("Setting up app %s (attempt %d)", app.app_name, attempt + 1)
      app.setup(env)
      logging.info("Successfully set up app %s", app.app_name)
      break
    except ValueError as e:
      logging.warning(
          "Failed to automatically setup app %s (attempt %d): %s.",
          app.app_name,
          attempt + 1,
          e,
      )
      if attempt == 2:
        logging.warning(
            "Failed to automatically setup app %s after 3 attempts.\n\nYou"
            " will need to manually setup the app.",
            app.app_name,
        )
