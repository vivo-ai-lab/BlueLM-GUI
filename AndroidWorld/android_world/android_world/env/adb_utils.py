...
def launch_app(
  ...
  if activity is None:
    ...
    return app_name
  # VLC's medialibrary fails to initialize when started via "am start -n"
  # directly (it hangs in the "Loading..." state). Monkey triggers the full
  # launcher path which initializes it correctly.
  if activity.startswith('org.videolan.vlc/'):
    package_name = extract_package_name(activity)
    response = issue_generic_request(
       ['shell', 'monkey', '-p', package_name, '-c', 'android.intent.category.LAUNCHER', '1'], env, timeout_sec=5)
    logging.info('Launching VLC via monkey, response: %r', response)
    return app_name
  start_activity(activity, extra_args=[], env=env, timeout_sec=5)
  for attempt in range(3):
    current_activity, _ = get_current_activity(env, timeout_sec=5)
    if current_activity and extract_package_name(current_activity) == extract_package_name(activity):
      logging.info('App %r is in the foreground after %d attempt(s).', app_name, attempt + 1)
      return app_name
    logging.warning(
        'App %r not in foreground after launch (attempt %d), retrying.',
        app_name,
        attempt + 1,
    )
    time.sleep(3.0)
    start_activity(activity, extra_args=[], env=env, timeout_sec=5)
  logging.error('App %r failed to reach the foreground after 3 attempts.', app_name)
  return app_name
