...
Helper scripts for building, running, and managing AndroidWorld Docker containers
live under [`scripts/`](scripts/). All scripts assume you run them from the
repository root.

### Build the Docker image

```bash
# Default image name: android_world:latest
...
This is equivalent to `docker build -t android_world:latest .` from the
repository root.

### Create a batch of containers

Creates one container per port in the given range. Each container maps its port
to the container's FastAPI server port (5000), i.e. `-p <PORT>:5000`.

```bash
# Create 64 containers on ports 5001..5064
...
|----------|---------|-------------|
| `IMAGE_NAME` | `android_world:latest` | Image to run |
| `CONTAINER_PREFIX` | `android_world` | Container name prefix (`android_world_<PORT>`) |
| `DRY_RUN` | `0` | Set to `1` to only print commands, without executing them |

Existing containers are skipped, so the script is safe to re-run.

### Remove containers

Removes containers by port range, or removes all containers with the prefix.

```bash
# Remove containers on ports 5001..5064
./scripts/remove_containers.sh 5001 5064

# Remove every container whose name starts with android_world_
...
Removes and recreates the containers for the given ports (useful after changing
the image).
...
# Restart several containers at once
./scripts/restart_containers.sh 5001 5003 5007 5010

# Use a custom image
IMAGE_NAME=android_world:dev ./scripts/restart_containers.sh 5001

# Dry run
DRY_RUN=1 ./scripts/restart_containers.sh 5001 5004
```

### Interact with a running environment

After building the image and starting one or more containers, you can use
`run_suite_on_docker.py` as a reference client to talk to the Android
environment server over HTTP:

```bash
# Build and start a single container first
./scripts/build_image.sh
docker run --privileged -p 5000:5000 -it android_world:latest
...
The client demonstrates the full server API flow: health check, reset, taking a
screenshot, executing an action, enumerating the suite (`task_list`,
`task_length`), initializing / scoring / tearing down a task, and closing the
environment. Replace the hardcoded `localhost:5000` base URL with
`http://<host>:<port>` to target any container in the pool.
